...
static const char ANSWER_STRINGS[ANSWER_STRINGS_MAX][20] = 
{
    ...
    "+CGMR: ",                  // 4
    "RING",                     // 5
    "+CIPSEND:",                   // 6 режим работы модул€: 0,1 или 4
    
    "CONNECT FAIL\r",                   //
    "+COPS: ",                  // 8 оператор симкарты:0,0"Beeline" или 0,0"Megafon" и т.д.
    "CONNECT OK\r",                   //
    "+CSQ: ",                   // 10 уровень сигнала:21,0 (где 21 может быть от 0 до 31 или 99)
    "CLOSE",                   // 
    "+CLIP:",                   // 12 вход€щий звонок:"+7XXXXXXXXXX",145,"","main",0
    "+CMTI: \"SM\",",           // 13 novoe sms; 
    "+CMGR:",                   // 14
    
    "+CMGS: ",                  // 15 сообщение об удачной передаче смс
    "+CUSD:",                   // 16 баланс:0,"56748939A766F567C",72
    "+ICCID: ",                 // 17 
    "+CNUM:",                   // 18 
    "RECV FROM:",                  // 19 режимы GPRS
    "+CFTPSGETFILE: ",                   // 20 
    ">",            // 21 и т.д.
    "+CFTPSLOGIN: ",               // 22
...
void Gsm::handler(void)
{
    ...
    switch (process){
        ...
        case PROCESS_REQUEST_BALANCE:
            ...
            break;
        case PROCESS_INIT_GPRS:
            gprs.processInitGprs();
            break;
        case PROCESS_INIT_CERTIFICAT:		
            gprs.processInitCertificat();
            break;
        case PROCESS_TRANSMIT_GPRS:
            gprs.processTransmitGprs();
            break;
        case PROCESS_CONNECT_WEB:
            gprs.processConnectWeb();
            break;
        case PROCESS_TRANSMIT_WEB:
            gprs.processTransmitWeb();
            break;
        case PROCESS_CONNECT_FTP:
            gprs.processConnectFtp();
            break;
        case PROCESS_FTP_GET:
            gprs.processFtpGet();
            break;
...
{
    static uint16_t posY = 0;   // позици€ провер€емого сообщени€ в таблице GSM команд
    static uint8_t posX = 0;    // позици€ провер€емого символа в таблице GSM команд
    static uint16_t pointTemp = 0;  // указатель чтени€ GSM сообщений в буфере(временный)
    static uint16_t point = 0;  // указатель чтени€ GSM сообщений в буфере
    uint16_t res = 0;
    ...
    while (posY < ANSWER_STRINGS_MAX)
    {
        ...
        if (ANSWER_STRINGS[posY][posX] == receiveArray[pointTemp]){
            ...
            if (ANSWER_STRINGS[posY] [posX] == 0){
                posX = 0;
                point = pointTemp;
                res = posY + 2;    // ¬озврат номера команды
                posY = 0;
            }
            if (res > 3){
                answerLast = res;
            ...
            return res;    // Ѕыло совпадение, обработка не закончена
        }
    ...
    // —овпадений не обнаружено

    if ((1<<answerLast) & ANSWER_CSQ                // уровень сигнала GSM
        || (1<<answerLast) & ANSWER_ICCID             // уровень сигнала GSM
        || (1<<answerLast) & ANSWER_CGMR         // ICCID
        || (1<<answerLast) & ANSWER_CNUM            // CNUM
        || (1<<answerLast) & ANSWER_CMTI            // пор€дковый номер вход€щего сообщени€
        || (1<<answerLast) & ANSWER_CMGR){          // чтение —ћ— сообщени€
        // забираем данные без кавычек
        if (isFirst){
            isFirst = false;
            answerDataPoint = 0;
    ...
    else if ((1<<answerLast) & ANSWER_CUSD      // баланс
        || (1<<answerLast) & ANSWER_COPS){      // им€ оператора
        
        // забираем данные в кавычках
        if (isFirst){
            isFirst = false;
            answerDataPoint = 0;
            for (int i=0; i<ANSWER_ARRAY_MAX; i++) answerData[i] = 0;
        ...
        if (isReadJson == false){
            ...
            if (receiveArray[pointTemp] == ','){
                isReadJson = false;
                answerLast = 0;
                gprs.answerCode = Convert.strToInt(answerCodeStr);
            }
            else{
    ...
    else if (((1<<answerLast) & ANSWER_HTTPREAD) && gprs.isAccessReadData){   // прин€тые данные с сервера
    ...
    else if ((1<<answerLast) & ANSWER_HTTPHEAD){   // прин€тые данные с сервера
