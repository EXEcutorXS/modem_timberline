...
void app_user_init(void)
{
    ...
    ke_timer_set(GAPM_DEVICE_READY_IND, TASK_APP, 10); //10Mºó¸´Î»
}
...
static uint8_t data_notify_onePackage(uint16_t param_length , uint8_t *param_payload, uint16_t task_id, uint8_t att_idx)
{
    ...
    /*
    struct user_notify_req *req = NULL;
    req = (struct user_notify_req*) malloc(param_length);
    req->send_param_length = param_length-2;
    req->att_idx = att_idx;
    memcpy(req->send_param_payload, &param_payload[2], param_length-2);
    //ke_msg_send_basic(USER_NOTIFY_REQ, prf_get_task_from_id(task_id), TASK_APP);
    ke_msg_send(req);
    for(int i=0; i<8; i++)
    {
        BT_handle();
...
uint8_t app_user_data_notify(uint16_t param_length , uint8_t *param_payload, uint8_t att_idx)
{
    ...
    while(param_length)
    {
        ...
        {
            data_notify/*_onePackage*/(param_length , param_payload, TASK_ID_USER, att_idx);
            param_length = 0;
        }
    }   
    return 0;
